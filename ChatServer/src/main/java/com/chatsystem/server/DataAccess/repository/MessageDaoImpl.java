...
public class MessageDaoImpl implements MessageDao {
    ...
    public boolean save(Message message) {
        return DatabaseManager.executeUpdate(
            "INSERT INTO messages (sender_id, receiver_id, message_type, content, file_path) VALUES (?, ?, ?, ?, ?)",
    ...
    public boolean update(Message message) {
        return DatabaseManager.executeUpdate(
            "UPDATE messages SET sender_id = ?, receiver_id = ?, message_type = ?, content = ?, file_path = ?, is_read = ?, sent_at = ?, is_deleted = ? WHERE message_id = ?",
            message.getSender_id(),
            message.getReceiver_id(),
            message.getMessage_type().name(),
            message.getContent(),
